...
begin
	# download the zip
	full_zip = UrlDownload.urldownload("https://osf.io/download/qd2s3/";
					compress=:zip,
					multifiles=true);
	
	# find the rds file within the zip
	file_ix_in_zip = findfirst([x.name == "data/processed/DF.l.inc.rds" for x in full_zip]);

	# load it (a bit convoluted, likely should be handled better by RData?)
	d_full =	RData.load((IOBuffer(read(seekstart(full_zip[file_ix_in_zip])))))

	# alternative, but not all columns are included here 
...
m_simple = fit(LinearMixedModel,
	...
	d_full,contrasts=Dict(
		:trial=>DummyCoding(base="neutr"),
...
# ╠═╡ show_logs = false
m_full = fit(LinearMixedModel,
	@formula(happiness~
					trial*image*condition+
					(1|ResponseId)+
					(trial*image*condition|lab)),
	d_full,contrasts=Dict(
		:trial=>DummyCoding(base="neutr"),
		:image=>DummyCoding(base="absentt"),
	:condition=>DummyCoding(base="mimicry"))
...
begin
res_full = coeftable(m_full) |> DataFrame
res_simple = coeftable(m_simple) |>DataFrame
res_combine = DataFramesMeta.outerjoin(res_simple,res_full;on=:Name,makeunique=true)

	select(res_combine,["Name","Coef.","Coef._1","Pr(>|z|)","Pr(>|z|)_1"])|>
	x->pretty_table(HTML,x,formatters = (
		ft_round(2,[2,3]),ft_printf("%08.8f",[4,5])))
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "9410b6ed72129818fe3dfbf487444f0773b6a1ae"
...
git-tree-sha1 = "31538466936a03a860d192d8db7bd1edb2179a91"
